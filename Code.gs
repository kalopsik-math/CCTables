...
//    var pt19_id = "1wya9phGL9OcLC7CDgkQ0YviPfjHcokCl";
//    
...
//        var f1 = DriveApp.getFolderById(pt19_id);
...
function spreadsheetBackup()
{
  ...
  var destFolder = DriveApp.getFolderById("1FTMjD_V5D18EioMJ-qqtrx1Ue7DeruNZ");
...
{
   var output_dir_id = "1O67JTE3BjOEtptAwFd5J1Po_oCKcdH0H";
    ...
    for (j=0; j<col2_data.length; j++)
    {
      if ( String(col2_data[j]) === "BEGIN" )
      {
        ...
        if (dsheet == null)
        {
          ...
          var values =[["ΟΜΑΔΑ", "ΠΕΡΙΓΡΑΦΗ", "ID", "ΤΕΜΑΧΙΑ", "ΚΟΣΤΟΣ/ΤΕΜ.", "ΕΝΔΕΙΚΤΙΚΟ ΜΟΝΤΕΛΟ", "ΕΓΚΡΙΝΕΤΑΙ"]];
          dsheet.getRange(1, 1, 1, 7).setValues(values);
          dsheet.getRange(1, 1, 1, 7).setFontWeight("bold");
        }
        ...
        var values = [[group, table_title, table_id, num_of_items, cost_per_item, indicating_model, accepted]];

        dsheet.getRange(dlastRow+1, 1, 1, 7).setValues(values);
        dsheet.autoResizeColumns(1, 7);
      }
    }
...
function newSheet()
{
  ...
  var new_sheet_name = Browser.inputBox("Δώστε το όνομα του νέου sheet: ΠΜΣ-ΚΑΕ (ένα φύλλο εργασίας ανά ΠΜΣ και ανά ΚΑΕ.");
...
function copyCurrentActiveSreadsheet()
{
